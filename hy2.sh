...
if [ "$PORT_HOP_ENABLED" = "yes" ]; then
    ...
    PORT_END=""
    while true; do
        ask PORT_END "跳跃范围结束端口（回车默认 $DEFAULT_PORT_END）" "$DEFAULT_PORT_END"
        [[ "$PORT_END" =~ ^[0-9]+$ ]] && [ "$PORT_END" -gt "$PORT" ] && [ "$PORT_END" -le 65535 ] && break
        warn "结束端口须大于 $PORT 且 ≤ 65535"
    done

    ask PORT_HOP_INTERVAL "跳跃间隔（回车默认 $DEFAULT_HOP_INTERVAL）" "$DEFAULT_HOP_INTERVAL"
...
while true; do
    ask CERT_CHOICE "请选择 [1-3]（回车默认 1）" "1"
    case "$CERT_CHOICE" in
        1)
...
while true; do
    ask SPEED_CHOICE "请选择 [1-2]（回车默认 2）" "2"
...
# ==========================================
SHARE_PORT="$PORT"
[ "$PORT_HOP_ENABLED" = "yes" ] && SHARE_PORT="${PORT_HOP_RANGE}"

if [ "$CERT_METHOD" = "acme" ]; then
    SNI="$ACME_DOMAIN"
    INSECURE_PARAM=""
else
    INSECURE_PARAM=$([ "$INSECURE" = "1" ] && echo "&insecure=1&pinSHA256=" || echo "")
fi
...
SHARE_LINK="hysteria2://${ENC_PASS}@${SHARE_HOST}:${SHARE_PORT}?sni=${SNI}${INSECURE_PARAM}#hy2"
