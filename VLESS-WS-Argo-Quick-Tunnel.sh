...
# ==========================================
# VLESS + WS + Argo Quick Tunnel
# ==========================================

WORKDIR="${WORKDIR:-${HOME}/vless-argo-temp}"
ARCH="$(uname -m)"
...
if [ -z "$ARGO_URL" ]; then
    cat "${WORKDIR}/cloudflared.log"
    fail_exit "获取 Argo 域名超时"
fi

ARGO_DOMAIN="$(echo "$ARGO_URL" | sed 's#https://##')"

# ----- 生成 VLESS 链接 -----
echo "[8/8] 生成 VLESS 链接..."

# VLESS 链接格式: vless://uuid@host:port?encryption=none&security=tls&sni=sni&type=ws&host=host&path=path#remark
VLESS_LINK="vless://${UUID}@${CF_PREFERRED_DOMAIN}:443?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=${WSPATH}#Argo-VLESS-${ARGO_DOMAIN:0:8}"
